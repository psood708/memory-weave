...
async def test_verify_session_returns_user_session(monkeypatch):
    ...
    db = AsyncMock()
    db.fetchrow = AsyncMock(return_value={"id": "uuid-1", "email": "a@b.com", "name": "Test"})
...
@pytest.mark.asyncio
async def test_verify_session_creates_user_when_not_found(monkeypatch):
    monkeypatch.setattr("memoryweave.auth.session.settings.auth_secret", SECRET)
    token = _make_token()
    ...
    db = AsyncMock()
    db.fetchrow = AsyncMock(return_value=None)  # user not found
    db.execute = AsyncMock()

    result = await verify_session(request, db)
    assert isinstance(result, UserSession)
    assert result.email == "a@b.com"
    db.execute.assert_called_once()  # INSERT was called
